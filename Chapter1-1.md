...
Swift는 언어를 위해 특별히 설계된 자체 메커니즘을 가질 때까지 동시성을 "차용" (“borrowed”) 했다.
...
- Thread explosion (스레드 폭발):
  ...
  너무 많은 동시 스레드를 만들려면 활성 스레드 간에 지속적으로 전환해야한다. 이렇게 하면 앱 속도가 느려진다.
...
이 계층은 계층에서 우선순위가 낮은 작업보다 우선순위가 높은 작업이 먼저 실행되는 큰 이점 및 보다 분명한 결과를 제공한다.
